...
const TestCaseArea = ({ problem }: { problem: Problem }) => {
  const { isRunning, runResult, submissionResult, isSubmitting } =
    useExecutionStore();

  const testCaseResults = submissionResult?.testCases || runResult?.testResults;
    ...
    <Card className="rounded-none h-full p-1">
      <Tabs defaultValue="case-1" className="h-full">
        ...
        {problem.testcases.slice(0, 3).map((example, index) => {
          const testResult = testCaseResults?.[index];
          const hasResult = testCaseResults && testResult;
                ...
                <CardContent className="space-y-2">
                  ...
                  {hasResult && (
                    ...
                    >
                      ...
                      <span className="text-sm">{testResult.stdout}</span>
                    </div>
                  )}
                </CardContent>
              </Card>
